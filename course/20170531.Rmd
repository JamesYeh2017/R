...
dist(rbind(x,y), method ="euclidean")   #計算點與點的距離
sqrt(sum((x-y)^2))
...
#數值變數作 scale(正規化) 離平均數有幾個sd
customer_s =scale(customer[,-1])  # 去掉第一欄(ID) 
?scale

round(mean(customer_s[,2]),3)
round(sd(customer_s[,2]),3)

#聚合式  bottom-up (hierarchial-clust)
?hclust
hc=hclust(dist(customer_s, method="euclidean"), method="ward.D2")   # hclust(dist點, 群合併方法) 
plot(hc,hang =-0.01, cex=0.7)  # hang 齊平, cex 字體大小

hc3 =hclust(dist(customer), method="single")
plot(hc3, hang =-0.01, cex=0.8)

#分裂式階層式  top-down (diana)
install.packages('cluster')
library(cluster)
?diana
dv =diana(customer_s, metric ="euclidean")   # 以點與點之距離衡量分群
summary(dv)    #分群結果
plot(dv)
...
str(customer_s)
set.seed(22)
fit =kmeans(customer_s, centers=4)   #分成4群
?kmeans
